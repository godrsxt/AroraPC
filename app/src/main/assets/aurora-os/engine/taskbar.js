...
const Taskbar = (() => {
  ...
  function renderStartApps(filter=''){
    ...
    const source = showingAllApps ? AppRegistry.all() : AppRegistry.pinned();
    const apps = source.filter(a => a.title.toLowerCase().includes(filter.toLowerCase()));
  ...
  function bindEvents(){
    ...
    const seeAllBtn = document.getElementById('start-see-all');
    if(seeAllBtn){
      seeAllBtn.addEventListener('click', () => {
        showingAllApps = !showingAllApps;
        seeAllBtn.textContent = showingAllApps ? 'Pinned' : 'All apps >';
        renderStartApps(document.getElementById('start-search-input').value);
      });
    }
