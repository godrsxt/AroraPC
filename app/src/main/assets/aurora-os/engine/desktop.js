...
const Desktop = (() => {
  ...
  function addIconTo(grid, id){
    ...
    el.addEventListener('contextmenu', (e) => {
      e.preventDefault();
      e.stopPropagation();
      if((typeof AppManager !== "undefined")) AppManager.hideFromDesktop(id);
    });
    grid.appendChild(el);
  }
  ...
  function renderIcons(){
    ...
    grid.innerHTML = '';
    const isHidden = (id) => (typeof AppManager !== "undefined") && AppManager.isHiddenFromDesktop(id);
    ...
    if((typeof AppManager !== "undefined")) AppManager.installedIds().forEach(id => { if(!isHidden(id)) addIconTo(grid, id); });
  }
