...
const WindowManager = (() => {
  ...
  // ---------------------------------------------------------
  // Third-party iframe app bridge (ADDITION)
  // ---------------------------------------------------------
  // Opens (installing first if needed) a third-party app whose manifest
  // was produced by AppManager.install(). Once registered it behaves like
  // any other app -- this is just a convenience wrapper around open().
  function openIframeApp(manifest){
    if(!AppRegistry.get(manifest.id) && (typeof AppManager !== "undefined")) AppManager.install(manifest);
    return open(manifest.id);
  }

  // contentWindow -> { winId, appId }. Only messages whose event.source is
  // a window WE created via an iframe we ourselves appended are trusted --
  // sandboxed iframes (no allow-same-origin) all report event.origin as
  // the literal string "null", so origin checks alone can't distinguish
  // them; tracking the actual contentWindow reference is what makes this
  // secure.
  const trustedIframeSources = new Map();

  function registerIframeSource(contentWindow, winId, appId){
    trustedIframeSources.set(contentWindow, { winId, appId });
  }
  function unregisterIframeSource(contentWindow){
    trustedIframeSources.delete(contentWindow);
  }

  // Small path guard: writes to the standard shared folders (Documents,
  // Downloads, Pictures) are honored as-is -- that's what the SDK example
  // expects when it saves to /Documents. Anything else gets confined to
  // the app's own folder under /Documents/AppData/<appId>/. Extend
  // SHARED_PREFIXES or replace this with a real manifest-permissions
  // check when you're ready for finer-grained control.
  const SHARED_PREFIXES = ['/Documents/', '/Downloads/', '/Pictures/'];
  function scopedPath(appId, requestedPath){
    const p = requestedPath || '/';
    if (SHARED_PREFIXES.some(prefix => p.startsWith(prefix))) return p;
  ...
  window.addEventListener('message', async (event) => {
    ...
    if(!msg.type || typeof msg.type !== 'string' || !msg.type.startsWith('aurora:')) return;
    ...
    try{
      if(msg.type === 'aurora:fileGet'){
        ...
        ok = false; error = `Unknown request type: ${msg.type}`;
      }
    }catch(e){
      ok = false; error = String((e && e.message) || e);
    }

    event.source.postMessage({ type:'aurora:response', requestId: msg.requestId, ok, result, error }, '*');
  });
  ...
  return {
    ...
    openIframeApp, registerIframeSource, unregisterIframeSource
  };
})();
